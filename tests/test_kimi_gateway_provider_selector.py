...
async def test_selector_uses_warmup_average_before_switching_to_latest_samples():
    ...
    await service.report_success("moonshotai", elapsed_seconds=4.0, output_units=2)
    await service.report_success("moonshotai", elapsed_seconds=3.0, output_units=3)

    moonshot_state = service._provider_states["moonshotai"]
    assert moonshot_state.current_unit_cost == pytest.approx(1.5)

    for provider_name in service.PROVIDER_NAMES[1:]:
        await service.report_failure(provider_name)

    await service.report_success("moonshotai", elapsed_seconds=10.0, output_units=5)

    assert moonshot_state.current_unit_cost == pytest.approx(2.0)
...
async def test_custom_model_send_injects_single_provider_options_for_kimi_gateway(
    ...
    assert gateway_payload["only"] == ["moonshotai"]
    assert gateway_payload["order"] == ["moonshotai"]
    assert gateway_payload["providerTimeouts"]["byok"] == {"moonshotai": 4000}
    assert result["selected_gateway_provider"] == "moonshotai"
...
async def test_custom_model_send_penalizes_retryable_status_and_switches_provider_on_next_call(
    ...
    assert captured_payloads[0]["providerOptions"]["gateway"]["only"] == ["moonshotai"]
    assert (
    ...
    assert result["selected_gateway_provider"] == "fireworks"
...
async def test_custom_model_send_releases_provider_on_non_retryable_4xx():
    ...
    moonshot_state = client._kimi_gateway_provider_selector._provider_states["moonshotai"]
    assert moonshot_state.explored is False
    assert moonshot_state.failure_penalty_seconds == pytest.approx(0.0)
...
async def test_custom_model_send_switches_provider_after_request_error():
    ...
    assert captured_payloads[0]["providerOptions"]["gateway"]["only"] == ["fireworks"]
    assert result["selected_gateway_provider"] == "fireworks"
