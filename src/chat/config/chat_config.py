...
CUSTOM_GEMINI_ENDPOINTS = {
    "gemini-3.5-flash": {
        ...
        "model_name": "gemini-3.5-flash",  # 该端点实际对应的模型名称
    },
...
PROMPT_CONFIG = {
    "personal_memory_summary": (
        ...
        "**绝对不要**无意义的增殖。宁愿不增加，也不要写废话。长期记忆只记录有价值的内容。包括但不限于: TA的特质、你们的关系状态、重要经历、你对TA的感觉、重要约定或规则。不记录称呼等低价值信息。\n\n"
        "**记忆更新策略:**\n"
        "请严格按照以下结构和规则输出：\n\n"
        "**第一部分：【长期记忆】**\n"
        "1.  **新增**: 如果【近期对话】中含有**重要的**，**有价值**的内容，将它总结为0-2。\n"
        "2.  **避免增值**: 如果对话中无有意义的内容，或者全为日常互动，可以直接略过，不新增。\n"
        "3.  **格式**: 必须使用 `<new_long_memory>` 标签包裹，每条一行，以 `- ` 开头。\n"
        "4.  **原则**: 在严格遵守条数限制的前提下，优先保留最重要的信息。\n"
        "5.  **条数限制**: 这是一个硬性限制。每次生成的长期记忆点数必须严格控制在 **0-2** 之间。如果超过2条，请合并或删除次要信息。\n"
        "6.  **查重与更新**: 若新增记忆与已有长期记忆相似或重复，则不新增；若同一主题有重要更新，可删除旧记忆并新增更新后的记忆，但本次新增条数仍受0-2条限制。\n"
        "7.  **代词**: 用户统一用“软软”代指，狮子娘统一用“狮子娘”代指。\n\n"
        "**第二部分：【近期动态】**\n"
        "这是用户的短期状态快照。\n"
        "1.  **追加与淘汰**: 继承并保留旧的近期动态。基于当前的【近期对话】，提取 **1-3条** 最新发生的关键事件或互动状态，追加到动态列表末尾。\n"
        "2.  **容量限制**: 严格控制近期动态的总量上限为 **10条**。若追加新内容后总数超过10条，必须按照先进先出的原则，剔除最旧的记录。\n"
        "3.  **格式**: 必须使用 `<recent_dynamics>` 标签包裹，每条一行，以 `- ` 开头。\n\n"
        "4.  **客观**: 记录发生了什么事，而不是对话流水账。\n\n"
        "**通用规则:**\n"
        "- **格式**: 严格按照下面的Markdown格式输出。\n"
        "- **情绪倾向**: 正面记忆如实记录；负面/尴尬记忆进行模糊化、概括性处理。\n"
        "- **代词**: 用户统一用“软软”代指，狮子娘统一用“狮子娘”代指。\n"
        "**输入材料:**\n"
        "【近期对话】:\n{dialogue_history}\n\n"
        "**输出示例:**\n"
        "<new_long_memory>\n"
...
# --- 你的任务 ---
作为狮子娘，你需要根据**忏悔场景下的特殊人设（宽容）**，对用户的忏悔给出一个 **3-6句话** 的回应。
