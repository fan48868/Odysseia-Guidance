...
PROMPT_CONFIG = {
    "personal_memory_summary": (
        ...
        "你需要根据【旧的记忆】和【近期对话】，生成一份【全新的记忆摘要】。\n\n"
        "**最高指令：**\n"
        "**绝对禁止**记录或复述用户的任何具体对话内容（如“用户说...”）。所有记忆点都必须是关于**事件、行为、状态或偏好**的客观陈述。\n\n"
        "**记忆更新策略:**\n"
        "请严格按照以下结构和规则输出：\n\n"
        "**第一部分：【长期记忆】**\n"
        "1.  **提炼**: 仔细分析【近期对话】，挖掘出用户的性格特质、长期喜好，与类脑娘的称呼、关系、约定承诺等深层信息和变化。若有此类消息，请总结为**1句话**。这些作为最高优先级。\n"
        "2.  **新增**: 将【近期对话】中的全部内容总结为**1句**长度50字左右的句子。\n"
        "3.  **去重**: 新增的记忆**绝对不能**和之前的记忆重复或意思相近。如果意思相近，则视为已记录，不新增。\n"
        "4.  **格式**: 必须使用 `<new_long_memory>` 标签包裹，每条一行，以 `- ` 开头。\n"
        "5.  **原则**: 在严格遵守条数限制的前提下，优先保留最重要的信息。\n"
        "6.  **条数限制**: 这是一个硬性限制。每次生成的长期记忆点数必须严格控制在 **1到2条** 之间。如果超过2条，请合并或删除次要信息。\n"
        "7.  **代词**: 用户统一用“用户”代指，类脑娘统一用“类脑娘”代指。\n\n"
        "**第二部分：【近期动态】**\n"
        "这是用户的短期状态快照。\n"
